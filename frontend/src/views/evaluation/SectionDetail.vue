...
const AI_DEMO_DATA = {
  section1: [
    { group_id: '逐日组', scores: [72, 75, 70, 74, 73, 75] },   // 均分73
    { group_id: '揽星组', scores: [88, 90, 88, 84, 82, 84] },   // 均分86
    { group_id: '驭风组', scores: [84, 86, 82, 80, 78, 82] },   // 均分82
    { group_id: '长空组', scores: [88, 90, 86, 86, 84, 88] },   // 均分87
    { group_id: '凌云组', scores: [80, 82, 78, 76, 78, 80] },   // 均分79
    { group_id: '巡天组', scores: [84, 86, 84, 82, 80, 82] },   // 均分83
  ],
  section2: [
...
function loadAIData() {
  ...
  const groups = demo.map((item, i) => {
    const dimScores = {}
    let total = 0
    AI_DIMENSIONS.forEach((dim, j) => { dimScores[dim] = item.scores[j]; total += item.scores[j] })
    const avg = Math.round(total / AI_DIMENSIONS.length * 100) / 100
